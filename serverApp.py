...
async def runModel(config: modelGuideFile, cleanup: BackgroundTasks):
    ...
    returnFiles.append(
        epidemic(
            community, requiredData, sessionID, cumulative=True, toolboxPath=toolboxPath
        )
    )
    returnFiles.append(
        epidemic(
    ...
    returnFiles.append(
        asir(community, requiredData, sessionID, toolboxPath=toolboxPath)
    )
    # If all else fails and no analyses were specified, run epidemic
    if not returnFiles:
        print("No analyses specified; defaulting to epidemic")
        returnFiles.append(
            epidemic(community, requiredData, sessionID, toolboxPath=toolboxPath)
        )
